...
Los diagramas dibujados a mano quedan viejos la semana después de exportarlos. Terraform
sigue alineado con producción. El PNG en la wiki, no.

[Diagrams with Python](https://github.com/agustinafassina/Diagrams.With.Python) es cómo
mantengo esas imágenes honestas: diagramas de infraestructura como código con
[mingrammer/diagrams](https://diagrams.mingrammer.com/). Corrés un script, sale un PNG,
commiteás los dos. Cambiás la topología, volvés a correr, la docs se mueve con ella.

## Qué hay en el repo

Cada carpeta es un script autocontenido con una ruta de salida predecible. Los scripts
resuelven config y salida desde su propio directorio, así que podés ejecutarlos desde
cualquier lado:

| Carpeta | Script | Qué modela |
| --- | --- | --- |
| `samples/` | `project-2.py` … `project-5.py` | Topologías AWS chicas (VPC, ALB, ECS, RDS) |
...
Hardcodear labels en Python sirve para un boceto. Cuando quiero ajustar textos sin tocar
Python, `samples/json-read.py` lee `config.json` (nombre de región, labels de componentes,
roles IAM, security groups) y arma el mismo layout Cluster/VPC:
...
Cambiás el JSON, volvés a correr, diagrama nuevo. Sirve cuando el mismo script atiende más
de un entorno, o cuando alguien que no vive en Python tiene que actualizar un label.

## Cómo ejecutar

Hace falta Python 3.x, [Graphviz](https://graphviz.org/download/) en el PATH, y
...
Diagrams también puede emitir un archivo fuente Graphviz junto al PNG. Se puede borrar; la
próxima corrida lo regenera.

## Por qué existe

Este repo es el acompañante visual del trabajo de infraestructura como código, en especial
las guías de
[Aws.Solutions.Architecture](https://github.com/agustinafassina/Aws.Solutions.Architecture).
En lugar de exports de Lucidchart en los que nadie confía, los diagramas viven en git junto
al Terraform que describen. Quien revisa ve la misma topología en el PR. Quien entra al
equipo regenera PNGs frescos en lugar de hacer zoom en una slide borrosa del trimestre
pasado.
